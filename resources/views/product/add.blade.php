    ...
    <div class="w-full relative mb-4">  
        ...
        <div class="flex-auto p-0 md:p-4">                                   
            <div class="mb-2">
                ...
                <input type="text" id="title" value="{{ old('name') }}" name="name" class="form-input w-full rounded-md mt-1 border border-slate-300/60 dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-2 focus:outline-none focus:ring-0 placeholder:text-slate-400/70 placeholder:font-normal placeholder:text-sm hover:border-slate-400 focus:border-brand-500 dark:focus:border-brand-500  dark:hover:border-slate-700" placeholder="Product Name" required>
                @if ($errors->has('name'))
            ...
            <div class="mb-2">
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <div class="col-span-1">
                        ...
                        <input type="title" name="price" value="{{ old('price')}}" id="price" class="form-input w-full rounded-md mt-1 border border-slate-300/60 dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-2 focus:outline-none focus:ring-0 placeholder:text-slate-400/70 placeholder:font-normal placeholder:text-sm hover:border-slate-400 focus:border-brand-500 dark:focus:border-brand-500  dark:hover:border-slate-700" placeholder="Price" required>
                        @if ($errors->has('price'))
            ...
            <div class="mb-2">
                ...
                <select id="gender" name='gender' value="{{ old('gender')}}" class="w-full rounded-md mt-1 border border-slate-300/60 dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-2 focus:outline-none focus:ring-0 placeholder:text-slate-400/70 placeholder:font-normal placeholder:text-sm hover:border-slate-400 focus:border-brand-500 dark:focus:border-brand-500  dark:hover:border-slate-700">
                    @if ($errors->has('gender'))
                        <span style="color:red"> {{ $errors->first('gender') }} </span>
                        @endif
                    <option  class="dark:text-slate-700">-- Gender --</option>
                    <option value="male" name='gender' class="dark:text-slate-700">Male</option>
                    <option value="female" name='gender' class="dark:text-slate-700">Female</option>
            ...
            <div class="mb-2">
                <label for="file" class="font-medium text-sm text-slate-600 dark:text-slate-400">Image</label>
                <input type="file" id="title" name="avatar" class="form-input w-full rounded-md mt-1 border border-slate-300/60 dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-2 focus:outline-none focus:ring-0 placeholder:text-slate-400/70 placeholder:font-normal placeholder:text-sm hover:border-slate-400 focus:border-brand-500 dark:focus:border-brand-500  dark:hover:border-slate-700" placeholder="Title" required>
                @if ($errors->has('avatar'))
                <span style="color:red"> {{ $errors->first('avatar') }} </span>
                @endif
            </div>
            <div class="">
                <button type="submit" class="px-2 py-2 lg:px-4 bg-brand  text-white text-sm  rounded hover:bg-brand-600 border border-brand-500">Add Product</button>
